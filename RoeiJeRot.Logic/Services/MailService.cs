...
namespace RoeiJeRot.Logic.Services
{
    ...
    public class MailService : IMailService
    {
        ...
        public void SendConfirmation(string email, string firstName, DateTime datum, TimeSpan tijd)
        {
            ...
            $"Je ontvangt deze mail omdat je een reservering hebt geplaatst voor een boot op {datum.ToString("d")} voor {tijd.TotalMinutes} minuten. In de bijlage vind u de afspraak die u kunt toevoegen aan uw eigen agenda." + Environment.NewLine + Environment.NewLine +
            ...
            str.AppendLine(string.Format("ATTENDEE;CN=\"{0}\";RSVP=TRUE:mailto:{1}", mailtje.To[0].DisplayName, mailtje.To[0].Address));
